...
Use `gemini_plan` for larger implementation goals that need decomposition. Supply the authorized `task.write_paths`, absolute workspace, full constraints or context IDs, and a planning token budget. It runs read-only. Inspect `gemini_plan_status`: only `plan_ready` contains validated steps and dependency waves. Dispatch each step’s returned `task` through spawn/batch after checking scope and budgets. Advance only after prerequisites actually complete; transfer predecessor findings explicitly with context packets or handoff. Planning and execution budgets are separate; there is no aggregate plan execution cap or automatic dispatcher. Structural validation is not test success. Cancel via `gemini_cancel`; create a new plan to revise constraints.
