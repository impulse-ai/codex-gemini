...
- [Context transfer and local peer communication](#context-transfer-and-local-peer-communication)
- [CLI usage](#cli-usage)
...
With autopilot enabled, implementation jobs receive a targeted progress nudge after eight consecutive model calls without a successful write. Monitoring continues after the first edit; each successful write resets the consecutive-call counter, nudge, and repeated-read baseline. After sixteen consecutive calls without edits—or twelve calls with at least three repeated reads since the last write—the server requests a final checkpoint and stops with `needs_attention`. A model's final completion claim without edits also requires a checkpoint and attention. No uninformed write is forced; a worker can identify missing information or explain why no change is needed. Normal token/time limits may stop work earlier.
