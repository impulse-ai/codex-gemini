...
func Server(m *Manager) *mcp.Server {
	s := mcp.NewServer(&mcp.Implementation{Name: "impulseai/codex-gemini", Version: "0.2.0"}, &mcp.ServerOptions{Instructions: "Delegate bounded work to Gemini workers. Publish reusable technical context once using gemini_publish_context, then pass context_ids to tasks. Use gemini_batch for up to 30 independent labeled tasks with disjoint write_paths. Select per-task thinking medium/high for complex topics. Workers can exchange brief messages and context IDs directly. Use gemini_handoff on stopped jobs to start fresh from reports and published context, reducing transcript replay; it never inherits write permissions. Use gemini_continue when full conversation continuity is necessary. Workers cannot execute shell commands; Codex must validate changes. Poll using gemini_wait and inspect gemini_status. Local persisted messages do not automatically wake completed workers. Do not send credentials in prompts."})
